...
def setup_search_engine(config: dict, dataset_adapter=None):
    ...
    if dataset_type in ["msa", "trec-car", "jeopardy", "legacy", "hdf5"]:
        ...
        searcher = SimpleBM25Searcher(
            dataset_adapter, k1=config["retrieval"].get("bm25_k1", 0.9), b=config["retrieval"].get("bm25_b", 0.4)
        )
    ...
    # For MS MARCO, use Pyserini/Lucene
        ...
        if not Path(index_path).exists():
            raise FileNotFoundError(f"Index not found at {index_path}. " f"Please run: python scripts/build_index.py")

        searcher = LuceneSearcher(index_path)
        searcher.set_bm25(config["retrieval"].get("bm25_k1", 0.9), config["retrieval"].get("bm25_b", 0.4))
...
def main(args):
    ...
    if config["rl_agent"].get("use_pretrained_embeddings", True):
        ...
        if embedding_type == "legacy":
            ...
            embedding_model = SentenceTransformer(config["rl_agent"].get("embedding_model", "all-MiniLM-L6-v2"))
    ...
    training_loop = RLTrainingLoop(
        config=config, pipeline=pipeline, train_dataset=train_dataset, val_dataset=val_dataset, test_dataset=test_dataset
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/msa_config.yaml", help="Path to config file")
